...
export default function ProfessorChatScreen() {
  ...
  // Guard: redireciona se não for professor
  useEffect(() => {
    if (!session) return;
    const tipo = session.tipo.toLowerCase();
    if (tipo === 'aluno') router.replace('/aluno/chat');
    else if (tipo !== 'professor') router.replace('/');
  }, [session?.tipo]);
  ...
  return (
    <View style={styles.container}>
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <View style={styles.info}>
                ...
                <Text style={styles.sub}>Aluno</Text>
              </View>
      ...
      <View style={styles.tabBar}>
        ...
        <TouchableOpacity style={styles.tabItem} onPress={() => router.push('/perfil?role=professor')}>
          {photo ? (
...
const styles = StyleSheet.create({
  ...
  blobTop: {
    ...
    backgroundColor: Colors.purple.dark, opacity: 0.5,
  },
  header: {
  ...
  logoutBtn: {
    ...
    borderWidth: 1, borderColor: Colors.purple.mid + '60',
  },
  ...
  avatar: {
    ...
    borderWidth: 1.5, borderColor: Colors.purple.mid + '60',
    overflow: 'hidden',
  },
